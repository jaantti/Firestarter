...
static void setcontrast(struct gspca_dev *gspca_dev)
{
	...
	if (sd->sensor == SENSOR_OV767x)
		sccb_reg_write(gspca_dev, 0x56, val);	/* contrast */
	else
...
static void setredblc(struct gspca_dev *gspca_dev)
{
	...
	val = sd->ctrls[REDBLC].val;
	sccb_reg_write(gspca_dev, 0x43, val); /* red balance */
}
...
static void setblueblc(struct gspca_dev *gspca_dev)
{
	...
	val = sd->ctrls[BLUEBLC].val;
	sccb_reg_write(gspca_dev, 0x42, val); /* blue balance */
}
...
static void sethue(struct gspca_dev *gspca_dev)
{
	...
	sccb_reg_write(gspca_dev, 0x01, val); /* Hue */
}
